...
export default function ReceiptScannerPage() {
  ...
  const { state, generateReceiptScannerRecipe, resetState, isLoading } =
    useRecipeGeneration();
  const { user } = useAuth();

  const handleGenerateRecipe = async () => {
    if (!imageBase64) {
      toast.error("Please upload a photo of your receipt!");
      return;
    }

    try {
      const recipe = await generateReceiptScannerRecipe({
        imageBase64,
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
      ...
      <div className="max-w-md mx-auto py-6 space-y-6">
        ...
        <div className="px-4">
          <InputWidgetTitle
            title="Take a photo of your grocery receipt"
            className="px-0"
          />
          <div className="mt-2">
            <ImageUploadArea
              ...
              label="Upload a photo of your receipt"
            />
          </div>
        </div>
